...
inline long long mysqrt(long long& n){ return std::floor(std::sqrt<>(n));}
...
void query_sqrt(std::vector<Node>& tree, int start_idx, int end_idx, int current_idx, int query_start, int query_end) {
    propagate(tree, current_idx, start_idx, end_idx);
    ...
    if (query_start <= start_idx && end_idx <= query_end) {
        ...
        if (mysqrt(tree[current_idx].min) == mysqrt(tree[current_idx].max)) {
            lazy1[current_idx] = mysqrt(tree[current_idx].min);
            propagate(tree, current_idx, start_idx, end_idx);
            return;
        }else if(tree[current_idx].min + 1 == tree[current_idx].max){
        // 최소 + 1 == 최대인 경우 같은값만큼 빼주는거처럼 작용한다.
            lazy2[current_idx] = (mysqrt(tree[current_idx].min) - tree[current_idx].min);
            propagate(tree, current_idx, start_idx, end_idx);
            return;
        }
    }
    int mid = (start_idx + end_idx) >> 1;
    query_sqrt(tree, start_idx, mid, (current_idx << 1), query_start, query_end);
    query_sqrt(tree, mid + 1, end_idx, (current_idx << 1) | 1, query_start, query_end);
