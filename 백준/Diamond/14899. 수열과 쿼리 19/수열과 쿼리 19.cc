...
long long lazy1[404040];
long long lazy2[404040];
struct Node {
    long long max;
    long long min;
    long long sum;
...
inline void propagate(std::vector<Node>& tree, int idx, int s, int e) {
    ...
    if(lazy1[idx] == INT_MIN){
        ...
        tree[idx].sum = (lazy1[idx] + lazy2[idx]) * (e - s + 1);
...
long long get_sum(std::vector<Node>& tree, int start_idx, int end_idx, int current_idx, int query_start, int query_end) {
    ...
    if (query_start <= start_idx && end_idx <= query_end) {
        return tree[current_idx].sum;
    }

    int mid = (start_idx + end_idx) >> 1;
    if (query_end <= mid) {
        return get_sum(tree, start_idx, mid, current_idx << 1, query_start, query_end);
    }
    else if (mid + 1 <= query_start) {
        return get_sum(tree, mid + 1, end_idx, (current_idx << 1) | 1, query_start, query_end);
    }
    else {
        auto left = get_sum(tree, start_idx, mid, current_idx << 1, query_start, mid);
        auto right = get_sum(tree, mid + 1, end_idx, (current_idx << 1) | 1, mid + 1, query_end);
        return left + right;
    }
}

long long get_min(std::vector<Node>& tree, int start_idx, int end_idx, int current_idx, int query_start, int query_end) {
    propagate(tree, current_idx, start_idx, end_idx);

    if(end_idx < query_start || query_end < start_idx) return 0;

    if (query_start <= start_idx && end_idx <= query_end) {
        return tree[current_idx].min;
    }

    int mid = (start_idx + end_idx) >> 1;
    if (query_end <= mid) {
        return get_min(tree, start_idx, mid, current_idx << 1, query_start, query_end);
    }
    else if (mid + 1 <= query_start) {
        return get_min(tree, mid + 1, end_idx, (current_idx << 1) | 1, query_start, query_end);
    }
    else {
        auto left = get_min(tree, start_idx, mid, current_idx << 1, query_start, mid);
        auto right = get_min(tree, mid + 1, end_idx, (current_idx << 1) | 1, mid + 1, query_end);
...
Node init_node(std::vector<Node>& tree, std::vector<long long>& values, int start_idx, int end_idx, int current_idx) {
    if (start_idx == end_idx) {
        tree[current_idx].sum = tree[current_idx].min = tree[current_idx].max = values[start_idx];
    ...
    tree[current_idx].sum = left.sum + right.sum;
    return tree[current_idx];
}
...
int main(int argc, char* argv[]) {
    ...
    std::vector<Node> tree(404040, Node());
    std::vector<long long> values(101010);
    int i, N, Q, q, l, r, v;

    N = readInt();
    Q = readInt();
    std::fill(tree.begin(), tree.end(), Node());
    std::fill(lazy1, lazy1+404040, INT_MIN);
    ...
    while (Q--) {
        ...
        if(q==1){
            ...
            writeInt(get_min(tree, 1, N, 1, l, r), '\n');
        }else{
            writeInt(get_sum(tree, 1, N, 1, l, r), '\n');
        }
    }
    Flusher();
    return 0;
