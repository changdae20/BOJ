...
for _ in range(Q):
    first, second, third, fourth = map(int,sys.stdin.readline().split())
    print(get_sum(1,1,N,min(first,second),max(first,second)))
    orig = get_value(1,1,N,third)
    if orig!=fourth: update(1,1,N,third, fourth-orig)
